...
void print (uint32_t start, uint32_t end, 
        uint32_t *start_values, uint32_t *end_values, uint32_t nibble_size)
{
    int i;
...
void check_range (uint32_t start, uint32_t end, 
        uint32_t start_values[][4], uint32_t end_values[][4],
        uint32_t count, uint32_t max_value, uint32_t nibble_size)
{
...
void
expand (uint32_t start, uint32_t end, bool debug, int mode)
{
    ...
    do {
        ...
        for (i = 0; i < 4; i++) {
            start_values[count][i] = (range_start >> (i * nibble_size)) & ((1<<nibble_size)-1);
            end_values[count][i] = (range_end >> (i * nibble_size)) & ((1<<nibble_size)-1);
        }
        if (debug) {
    ...
    check_range(start, end, start_values, end_values, count, max_val, nibble_size);
}
...
int main (int argc, char** argv)
{
    ...
    if (argc == 3) {
        ...
        for (i = 0; i < 5000; i++) {
            ...
            uint32_t end = start + 
                (rand() % (params[FOUR_BIT].max_value-start));
